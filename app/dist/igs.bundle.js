...
__igsReExport(exports, __igsReExportSource12, [["createImmersiveGalgameSystemReaderHost","createImmersiveGalgameSystemReaderHost"]]);
...
__igsReExport(exports, __igsReExportSource20, [["writeLegacyImmersiveGalgameSystemSettings","writeLegacyImmersiveGalgameSystemSettings"]]);
});
...
const { readLegacyImmersiveGalgameSystemSettings, writeLegacyImmersiveGalgameSystemSettings, resolveLegacyReaderMode } = require("src/storage/legacy-igs.js");
...
const { createImmersiveGalgameSystemReaderHost } = require("src/visual/igs-ui/reader-host.js");
...
const { createImmersiveGalgameSystemCompatApi } = require("src/api/igs-compat.js");
const API_GROUPS = [
    'imageProviders',
...
const { parseSceneText } = require("src/scene/text-parser.js");
const { buildImmersiveGalgameSystemTextPayload } = require("src/scene/message-source.js");
const READER_MODES = Object.freeze(['pc', 'mobile', 'web', 'fullscreen']);
function createImmersiveGalgameSystemCompatApi(app) {
    return {
...
__igsDefine(exports, "createImmersiveGalgameSystemCompatApi", () => createImmersiveGalgameSystemCompatApi);
});
__igsRegister("src/storage/legacy-igs.js", function(module, exports, require) {
const LEGACY_IGS_KEYS = Object.freeze({
...
function readLegacyImmersiveGalgameSystemSettings(storageLike, preferredMode) {
    const result = {
        ok: true,
        bridge: {},
        displayMode: '',
...
function writeLegacyImmersiveGalgameSystemSettings(storageLike, nextState = {}) {
    if (!isStorageLike(storageLike)) {
        return {
            ok: false,
            reason: 'missing-storage-like',
...
function normalizeLegacyImmersiveGalgameSystemSettings(raw) {
...
__igsDefine(exports, "readLegacyImmersiveGalgameSystemSettings", () => readLegacyImmersiveGalgameSystemSettings);
__igsDefine(exports, "writeLegacyImmersiveGalgameSystemSettings", () => writeLegacyImmersiveGalgameSystemSettings);
__igsDefine(exports, "normalizeLegacyImmersiveGalgameSystemSettings", () => normalizeLegacyImmersiveGalgameSystemSettings);
__igsDefine(exports, "resolveLegacyReaderMode", () => resolveLegacyReaderMode);
__igsDefine(exports, "LEGACY_IGS_KEYS", () => LEGACY_IGS_KEYS);
...
function buildImmersiveGalgameSystemTextPayload(message, options = {}) {
...
__igsDefine(exports, "buildImmersiveGalgameSystemTextPayload", () => buildImmersiveGalgameSystemTextPayload);
...
const { buildImmersiveGalgameSystemTextPayload, getMessagePrimaryText, normalizeSourceFilter, normalizeVirtualRegex } = require("src/scene/message-source.js");const { resolveSceneStateAtIndex, lookupSceneAssetUrls } = require("src/scene/scene-directives.js");const { getOriginalReaderHtml, getOriginalReaderSource, getOriginalReaderStyleText, ORIGINAL_READER_REQUIRED_SELECTORS, ORIGINAL_READER_STYLE_CONTRACT } = require("src/visual/igs-ui/original-reader-source.js");const { getSettingsShellTemplate } = require("src/visual/igs-ui/settings-shell.js");const { getSettingsStyleText } = require("src/visual/igs-ui/settings-style.js");const { getSettingsTabTemplate, SETTINGS_TAB_DEFS } = require("src/visual/igs-ui/settings-tabs.js");const { getReaderModeIcon } = require("src/visual/igs-ui/icons.js");const { DEFAULT_IMAGE_API, DEFAULT_PINNED_TOOLBAR_BUTTONS, DEFAULT_SCENE_PROMPT_RULE, READER_SETTINGS_SCHEMA_VERSION, SETTINGS_PANEL_REQUIRED_SELECTORS, SETTINGS_PANEL_TAB_CONTRACT, TOOLBAR_ACTIONS, IGS_THEME_PRESETS } = require("src/visual/igs-ui/reader-host-constants.js");const { cloneData, esc, firstDefined, firstNonEmptyString, firstRenderableText, normalizeBoolean, normalizeFiniteIndex, normalizeFiniteNumber, normalizeNullableNumber, normalizeOpacity, stripSpeakerPrefix, toHex } = require("src/visual/igs-ui/reader-value-utils.js");const { checkbox, colorInput, field, renderCharacterAssetList, renderPinnedButtons, renderSceneAssetList, renderTemplate, secretInput, segmentedInput, selectInput, textInput, textareaInput, numberInput, disabledAttr, modelPicker } = require("src/visual/igs-ui/settings-fields.js");const { applyImageCountOverride, buildImageActionContext, buildProgressText, countBoundImageSlots, normalizePollAttempts, normalizePollInterval, normalizeSnapshotImageState, resolveSegmentImageIndex, shouldPollReaderImages, waitForReaderImagePoll } = require("src/visual/igs-ui/reader-image-state.js");const { attachSettingsViewportEvents, clearChildren, detachSettingsViewportEvents, ensureImageLoadingSpinner, ensureStyleTag, getOwnerWindow, getRootDocument, removeImageLoadingSpinner, syncSettingsViewportVars, unmountNode } = require("src/visual/igs-ui/reader-dom-utils.js");const { buildTextSegments, getPath, normalizeBtnOrder, normalizeHiddenButtons, normalizePinnedButtons, normalizeReaderMode, normalizeSettingsTab, normalizeSettingsValue, normalizeSpriteLayouts, setPath } = require("src/visual/igs-ui/settings-normalize.js");const { clearReaderModeRuntime, exitDocumentFullscreen } = require("src/visual/igs-ui/reader-runtime.js");const { enterSpriteEditMode } = require("src/visual/igs-ui/sprite-edit.js");const { handleSettingsAction: runSettingsAction } = require("src/visual/igs-ui/settings-actions.js");const { applyReaderSnapshotToDom, applyToolbarState, buildFallbackReaderOverlay, buildFallbackSettingsOverlay } = require("src/visual/igs-ui/reader-dom-render.js");
function createImmersiveGalgameSystemReaderHost(options = {}) {
    const state = {
        activeReader: null,
        activeSettings: null,
    };
...
__igsDefine(exports, "createImmersiveGalgameSystemReaderHost", () => createImmersiveGalgameSystemReaderHost);
});
...
const DEFAULT_IMAGE_API = Object.freeze({
    ...
    availableModels: [],
    modelsFetchedAt: '',
});const IGS_THEME_PRESETS = Object.freeze({
...
__igsDefine(exports, "IGS_THEME_PRESETS", () => IGS_THEME_PRESETS);
...
const { ORIGINAL_READER_ICONS, ORIGINAL_READER_TOOLBAR_BUTTONS } = require("src/visual/igs-ui/original-reader-source.js");const { TOOLBAR_ACTIONS } = require("src/visual/igs-ui/reader-host-constants.js");const { ensureImageLoadingSpinner, getOwnerWindow, readElementHeight, readElementWidth, removeImageLoadingSpinner } = require("src/visual/igs-ui/reader-dom-utils.js");const { computeLineHeight, normalizeOpacity, igsDebug } = require("src/visual/igs-ui/reader-value-utils.js");const { renderDialogueHtml, resolveActiveTheme, resolveSpriteLayout } = require("src/visual/igs-ui/settings-normalize.js");const { applyReaderModeRuntime } = require("src/visual/igs-ui/reader-runtime.js");
...
const createImmersiveGalgameSystemReaderHost = __igsEntry["createImmersiveGalgameSystemReaderHost"];
...
const writeLegacyImmersiveGalgameSystemSettings = __igsEntry["writeLegacyImmersiveGalgameSystemSettings"];
...
export { PRESET_STORE_KEY, VISUAL_MODES, attachPublicApi, bootstrapIGS, createGenerationQueue, createImmersiveGalgameSystemReaderHost, createLayerController, createMagicWandEntry, createMemoryStorage, createPresetGroup, createPresetRegistry, createPresetStore, createPromptAdapter, createPublicApi, createReaderState, createResourceCache, createShujukuClient, createStageModel, createStageRenderer, createTavernHelperAdapter, destroyIGS, dispatchImportBundle, importPresetBundle, parseSceneText, resolveScene, resolveVisualMode, writeLegacyImmersiveGalgameSystemSettings };
