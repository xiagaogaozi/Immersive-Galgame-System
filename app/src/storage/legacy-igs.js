export const LEGACY_IGS_KEYS = Object.freeze({
    bridge: 'igs_igs_bridge_config',
...
export function readLegacyImmersiveGalgameSystemSettings(storageLike, preferredMode) {
    const result = {
        ok: true,
        bridge: {},
        displayMode: '',
    ...
    const bridgeRecord = readJsonStorage(storageLike, LEGACY_IGS_KEYS.bridge);
    if (!bridgeRecord.ok) return bridgeRecord;
    result.bridge = normalizeLegacyImmersiveGalgameSystemSettings(bridgeRecord.value);

    const displayModeRecord = readStringStorage(storageLike, LEGACY_IGS_KEYS.displayMode);
    ...
    for (const mode of LEGACY_READER_MODES) {
        const record = readJsonStorage(storageLike, `${LEGACY_IGS_KEYS.readerPrefix}${mode}`);
        if (!record.ok) return record;
        result.readerSettingsByMode[mode] = normalizeLegacyImmersiveGalgameSystemSettings(record.value);
    }
...
export function writeLegacyImmersiveGalgameSystemSettings(storageLike, nextState = {}) {
    if (!isStorageLike(storageLike)) {
        return {
            ok: false,
    ...
    try {
        storageLike.setItem(LEGACY_IGS_KEYS.bridge, JSON.stringify(normalized.bridge));
        storageLike.setItem(LEGACY_IGS_KEYS.displayMode, normalized.displayMode);
        for (const mode of LEGACY_READER_MODES) {
            storageLike.setItem(`${LEGACY_IGS_KEYS.readerPrefix}${mode}`, JSON.stringify(normalized.readerSettingsByMode[mode]));
        }
        return {
            ok: true,
            legacy: normalized,
...
export function normalizeLegacyImmersiveGalgameSystemSettings(raw) {
...
function normalizeLegacySnapshot(raw) {
    const bridge = normalizeLegacyImmersiveGalgameSystemSettings(raw.bridge);
    ...
    for (const mode of LEGACY_READER_MODES) {
        ...
        readerSettingsByMode[mode] = normalizeLegacyImmersiveGalgameSystemSettings(source);
    }
