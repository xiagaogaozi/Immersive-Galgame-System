...
import { buildImmersiveGalgameSystemTextPayload } from '../scene/message-source.js';

const READER_MODES = Object.freeze(['pc', 'mobile', 'web', 'fullscreen']);

export function createImmersiveGalgameSystemCompatApi(app) {
    return {
        openSettings(options = {}) {
            ...
            if (!app.immersiveGalgameSystemUi || typeof app.immersiveGalgameSystemUi.openSettings !== 'function') {
                return { ok: false, reason: 'settings-ui-not-mounted', options: payload };
            }
            return app.immersiveGalgameSystemUi.openSettings(payload);
        },
...
function getLegacySettings(app) {
    if (!app || typeof app.getLegacyImmersiveGalgameSystemSettings !== 'function') {
        return {
            ok: true,
            bridge: {},
            displayMode: '',
    ...
    return app.getLegacyImmersiveGalgameSystemSettings();
}
...
function buildReaderPayload(app, message, messageId, readerMode) {
    ...
    const immersiveGalgameSystemText = buildImmersiveGalgameSystemTextPayload(message, {
        sourceFilter: bridge.sourceFilter,
        virtualRegex: bridge.virtualRegex,
    });
    const textScene = parseSceneText(
        immersiveGalgameSystemText.formattedText || immersiveGalgameSystemText.visibleText || immersiveGalgameSystemText.cleanedRaw || '',
        { messageId },
    );
    const readerText = Array.isArray(immersiveGalgameSystemText.textSegments) && immersiveGalgameSystemText.textSegments.length
        ? immersiveGalgameSystemText.textSegments.join('\n')
        : textScene.text;

    return {
        ...immersiveGalgameSystemText,
        message,
        messageId,
        mode: readerMode,
        viewerMode: readerMode,
...
function openReaderUi(app, payload, refreshed) {
    if (!app.immersiveGalgameSystemUi || typeof app.immersiveGalgameSystemUi.openReader !== 'function') {
        return refreshed;
    }
    const reader = app.immersiveGalgameSystemUi.openReader(payload, { mode: payload.mode });
    return {
        ...refreshed,
