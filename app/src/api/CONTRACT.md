...
VN.api.sceneRegexPresets.register(preset);
VN.api.textFilterPresets.register(preset);
VN.api.textFormatPresets.register(preset);
...
- `exportAll()`：导出当前分组全部预设，保持 `vn-import-bundle` 结构。
...
- 新二创能力优先挂在 `VN.api.*` 下，不继续扩散旧 `VisualNovelBridge` 调试入口。
