(function () {
    ...
    try { root.IGS_DEBUG = true; } catch (igsDebugError) { /* ignore */ }
    const doc = getRootDocument();

    if (!doc) {
        console.warn('[IGS Loader] 未找到可访问的文档，无法加载沉浸式galgame系统。');
        return;
    }
    ...
    load().catch((error) => {
        ...
        try {
            if (typeof root.alert === 'function') root.alert(`沉浸式galgame系统 启动失败：${error && error.message || String(error)}`);
        } catch (alertError) {
    ...
    async function load() {
        ...
        throw lastError || new Error('没有可用的沉浸式galgame系统远程 bundle。');
    }
    ...
    function createLoaderMagicEntry(candidateDoc) {
        ...
        button.setAttribute('title', '沉浸式galgame系统正在加载');
        button.setAttribute('aria-label', '沉浸式galgame系统正在加载');
    ...
    function notifyLoaderPending() {
        const message = '沉浸式galgame系统仍在加载远程脚本，请稍等几秒后再点。';
        try {
    ...
    function injectScript(src) {
        ...
        return new Promise((resolve, reject) => {
            ...
            script.src = src;
            script.onload = () => {
                console.info('[IGS Loader] 沉浸式galgame系统 bundle 已加载。', src);
                resolve(script);
            };
            script.onerror = () => {
    ...
    function reconcileExistingRuntime() {
        ...
        if (api && typeof api.ensureMagicWandEntry === 'function') {
            ...
            console.info('[IGS Loader] 沉浸式galgame系统已加载，已重扫魔法棒入口。', result);
            scheduleMagicWandEnsure();
