...
`app/dist/vn.bundle.js` 是给 loader 远程加载的自包含主程序文件。它不能只是 `import ../src/index.js` 的转发入口，否则 cache bust 只会刷新入口文件，浏览器或酒馆仍可能复用旧的源码子模块。
...
- 最终给用户导入的是版本化中文发布文件 `loader/酒馆助手脚本-Visual Novel（自动更新） v<当前版本>.json`，不是 `app/dist/vn.bundle.js`。`loader/vn-loader.json` 保留为固定内部入口和自动化校验基准。

## loader JSON 格式

`loader/vn-loader.json` 和版本化中文发布文件必须都是 JS-Slash-Runner 可导入脚本：
...
`loader/vn-loader.js` 只做重复加载检查、版本解析、CSS 注入、JS 注入和错误提示，不承载业务 UI。
`loader/vn-loader.json` 不提供额外酒馆助手按钮；正式入口必须由主程序注入酒馆魔法棒菜单。
...
loader 默认追踪 GitHub API 返回的 `main` 最新提交哈希，并优先加载该提交对应的不可变 bundle；酒馆端导入一次 loader 后，后续只要仓库 `main` 更新，就会继续拉取最新 bundle，不再要求 loader 源码同步改版本号。loader 会在远程主程序加载完成前先显示临时魔法棒入口，正式运行时加载后替换为正式入口。需要临时锁定旧版本或测试分支时，也可以手动设置 `window.VN_LOADER_REF` 或 `window.VN_LOADER_CONFIG.ref`；指定的非 `main` ref 不可用时会 fallback 到 `@main`。
...
```powershell
node -e "const fs=require('fs'); const js=fs.readFileSync('loader/vn-loader.js','utf8'); const j=JSON.parse(fs.readFileSync('loader/vn-loader.json','utf8')); if(j.type!=='script'||j.content!==js||!j.content.includes('vn.bundle.js')) throw new Error('bad loader json');"
