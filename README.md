...
- 当前项目版本 `v0.18.0`：阅读器交互升级——「刷新图位」改为「刷新」(并行重扫正文+图片，刷新后回到第一页)；输入框+发送按钮+上方分割线只在最后一页显示；状态行(进度)移到气泡底部居中、最后一页不显示(仍受「显示状态行」开关控制)；工具栏新增「第一页(<<)」「最后一页(>>)」按钮；工具栏默认顺序与命名调整为 上一轮/第一页/上一页/下一页/最后一页/下一轮/重新生图/保存图片/隐藏对话框/调整立绘/刷新/设置。
