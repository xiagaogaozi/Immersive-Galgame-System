...
- 当前项目版本 `v0.17.2`：删除立绘槽展开态的「xx组词」标题；删除「为所有角色按情绪组名补建立绘槽」(↑)按钮及其 handler；修复在立绘槽改名时无同名检查直接覆盖丢失的 bug——现改名先查该角色同名槽和词库同名组（撞名弹窗阻止），通过后同时改角色槽名+词库同名情绪组组名（全局，所有角色用到该组名的槽一起改）。
