...
- 当前项目版本 `v0.8.1`：新增调试版 loader（IGS_DEBUG 开关 + [DEBUG-sprite] 探针），移除无效的「调试日志」开关。
...
- 最终酒馆导入形态：`loader/酒馆助手脚本-沉浸式galgame系统（自动更新） v0.8.1.json`；`loader/igs-loader.json` 保留为固定内部入口和自动化校验基准。
- 原版脚本来源：原版备份目录。
- 策划书版本归档目录：`plan/`
...
- 修复历史场景标签在本地正文替换规则未命中时未清洗的问题：正文格式化新增 `*-scene` 兜底识别，场景素材提取同步兼容。
- 修复未绑定图片槽位导致阅读器背景层持续 loading：未生成或未匹配图片时不再自动显示无限 spinner，手动刷新仍保留临时加载提示。
...
- 设置面板背景名/角色名/情绪名旁加 SVG 铅笔编辑名称按钮（prompt 弹窗）+ SVG 垃圾桶删除按钮。
...
- 完善运行时边界：公开全局对象、CSS/DOM 槽位、存储前缀和导入后缀保持 `IGS` / `igs` 体系。
- 发布链路稳定输出 `app/dist/igs.bundle.js`、`app/dist/igs.bundle.css`、`loader/igs-loader.js` 与 `loader/igs-loader.json`。
- 最终导入件固定为 `loader/酒馆助手脚本-沉浸式galgame系统（自动更新） v0.3.17.json`，loader 继续从 GitHub `main` 最新提交加载远程 bundle。

### v0.3.14 - 2026-06-13

- 补齐活文档、AI 工作流、发布说明和 UI 契约中的原版参考说明。
- 魔法棒入口、运行时公开名、dist manifest 和用户可见导入文件保持同一发布口径。
- 最终导入件固定为 `loader/酒馆助手脚本-沉浸式galgame系统（自动更新） v<当前版本>.json`；`loader/igs-loader.json` 继续作为固定内部入口和自动化校验基准。

### v0.3.13 - 2026-06-13

- 按 `plan/v0.3.13-原版IGS安全楼层取图与image占位绑定施工图.md` 收紧原版 IGS 兼容取图范围：`reader-image-service.js` 现已在存在 `<image>` 图位时强制要求当前楼层作用域，找不到当前楼层根时直接返回空图位，不再退回整页扫描。
...
- 按 `plan/v0.3.5-原版IGS剩余重构审计与补全策划书.md` 补齐原版 IGS 剩余重构缺口：新增 `app/src/generated-images/image-api-client.js`、`dom-image-candidates.js` 与 `providers/dom-generic-provider.js`，把原版的模型读取、真实 NAI 生图、DOM 插图探测和通用外部适配收口到 `generated-images` 运行时。
...
- 按 `plan/v0.3.3-原版IGS分页拖拽与AI楼层修复施工图.md` 完成运行时代码修复：`buildTextSegments()` 现已对齐原版 IGS 的按 `\n+` 分段语义，一行多句保持单页，单换行多段拆成多页。
...
- 新增 `plan/v0.3.3-原版IGS分页拖拽与AI楼层修复施工图.md`，将当前发现的 IGS parity 差异固化为可执行施工图。
...
- `app/src/host/tavern-helper-adapter.js` 新增 `listMessages()`、`getAdjacentMessage()`、`jumpToMessage()`、`findRegenerateButton()`，并按原版 IGS 语义补齐可读楼层筛选、消息归一化和重绘按钮定位。
...
- 本轮仍不修改原版 `projects/<当前本地项目文件夹> 原版备份/**`；上一轮/下一轮跨楼层真实 DOM 图片缓存与真实 provider 重画/保存仍需后续在 host/generated-images 层继续补齐。
...
- 归档 `plan/v0.2.12-原版IGS可用性修复施工图.md`，当时目标是恢复原版 `fa-book-open` 魔法棒入口并修复阅读器把酒馆宿主 HTML 当正文显示的问题；入口文案保持 `Immersive Galgame System`。
...
- 删除 `启动 IGS`、`重扫入口` 两个酒馆助手按钮；正式用户入口只保留酒馆魔法棒菜单里的 `Immersive Galgame System`。
...
- 新增 `app/src/storage/legacy-igs.js`，只读读取 `igs_igs_bridge_config`、`igs-reader-settings-v9-*` 和 `igs-display-mode`。
...
- 新增 `app/fixtures/igs/*`、`docs/IGS_MIGRATION.md` 与 gate 测试，固定第一阶段兼容基线。
