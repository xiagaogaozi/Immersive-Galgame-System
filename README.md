...
- 当前项目版本 `v0.22.3`：修复4个问题——①场景/时间/天气背景查表改为「精确→组归约→默认」三级兜底（对齐情绪词逻辑，AI 写细分词可命中组名 URL）；新增 `{{scene_groups}}`/`{{time_groups}}`/`{{weather_groups}}` 占位符（默认模板已含，可 DIY 删除）；②立绘保存后回初始位置回归（空 mood 存 `mode::char` 与读取侧对齐）；③数据库表格色跟随 glassOpacity（CSS 变量驱动表头 sticky 背景）；④数据库面板默认右上角缩小。
