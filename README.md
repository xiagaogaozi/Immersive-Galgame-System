...
- 当前项目版本 `v0.11.2`：修复立绘情绪组名槽不显示的真 bug——根因有两层：① `scene-directives.js` 的三个 directive 正则用了行尾锚点 `\]$`，导致带翻译尾巴（如 `[igs-char:小林海斗|淡然|まさか…]*（怎么可能）*`）的标签整行不匹配、directive 全部漏提取，去掉 `$` 锚点改为匹配到闭合括号即可；② 立绘 mood 原先按「气泡出现顺序」配对 directive，正文里的 `<image>` 块、斜体旁白、合并/剥离段会让序号整体错位，取到隔壁情绪词。改为用 speaker + 台词文本指纹匹配定位 directive（剥离翻译尾巴后子串比对），不再依赖序号。立绘按情绪组名归约取图现已稳定命中。
