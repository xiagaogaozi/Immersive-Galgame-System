...
- 当前项目版本 `v0.16.0`：取消情绪词必须 2-3 汉字的限制（非空即可，仍跨组去重）；场景页重构为「启用开关 → 格式规则注入 → 选择预设 → 子标签栏（场景素材/角色立绘）→ 子页面」；对话主题从场景页搬到阅读器页，存储从全局 `bridge.vnTheme` 迁到按模式存的 `readerSettings.vnTheme`（旧全局数据兼容兜底），且仅在启用场景素材模式时可改；「应用到模式」新增「默认」选项（改设置写入全部四个模式桶），并拆掉切「应用到模式」自动切换显示模式的联动（保留切显示模式时「应用到模式」跟随）。
