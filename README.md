...
- 当前项目版本 `v0.13.0`：立绘位置 key 改为 `mode::角色::情绪`，各情绪可独立调整位置；新增「统一角色立绘位置」开关（默认关，开启后一次调整应用到该角色所有情绪槽）；立绘位置纳入场景预设导入/导出；修复旁白页立绘随机选错情绪图、且无法移动保存的 bug（旁白页现沿用前一个 char/thought 标记的角色+情绪）。
