...
- 当前项目版本 `v0.23.16`：设置 → 阅读器新增「图片亮度」百分比下拉（50%~100%，默认 88%）。背景图亮度此前由 CSS 硬编码 `brightness(.88)` 全局压暗，现改为按 `readerSettings.imgBrightness` 用 inline style 控制，想要原图亮度可调到 100%。
