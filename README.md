...
- 当前项目版本 `v0.20.4`：修复 floating 模式下对话框高度≥160px 分割线消失（speaker/divider 加 `flex-shrink:0`）；「应用到模式」独立桶——「默认」现有专属存储桶 `igs-reader-settings-v9-default`，不再分发写入全部四个桶；切换「切换模式」不再联动「应用到模式」选项。
