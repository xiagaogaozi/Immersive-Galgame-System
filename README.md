...
- 当前项目版本 `v0.20.3`：修复阅读器读不到第三方 DOM 过滤插件（如 Veridis 关键词过滤）写回 `mes` 的内容——`buildReaderSnapshot` 现在优先读 `payload.message.raw`（ST 活引用）而非 normalized wrapper 的缓存字段 `rawHtml`/`text`。
