...
- 当前项目版本 `v0.14.0`：新增「按句号自动分页」开关（阅读器页）——启用场景素材模式时仅对旁白行按句号 `。` 断页、对话/心理整段保留；未启用时全部正文按句号断页；分页在 `buildIgsTextPayload` 内统一处理，`textSegments` 与 `segmentImageSlots` 同步对齐。修复发送消息报 `missing-send-api`：`typeAndSend` 在 helper 无 `typeAndSend`/`setInputText`+`send` 时回退到宿主 DOM 模拟发送（写 `#send_textarea` + 触发 input/change + 点 `#send_but`）。收紧场景素材模式下角色名到气泡的间距（按字号抵消首行行内留白）。删除无实际行为的「留在当前模式」开关。
