...
- 当前项目版本 `v0.21.0`：背景场景词库+预览图——场景名/时间/天气三层均新增展开折叠（▼/▲）、词库 tag（带 × 删除/+ 添加/跨层全局重复词 confirm 迁移）、预览缩略图；三层组名同名改名阻止；天气条目从字符串升级为 object（向后兼容）；提示词注入新增场景/时间/天气三条约束（空间限定、笼统时段、天气类型词）。
