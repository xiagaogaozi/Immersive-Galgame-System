...
- 当前项目版本 `v0.17.1`：修复 v0.17.0 立绘情绪槽展开按钮点击无反应——展开态 key 在渲染端用 NUL 字节、在 action 端用普通空格，分隔符不一致导致 Set 命中失败；现两端统一为 `"\x00"` 转义（消除源码里的真实 NUL 字节），并补端到端测试覆盖「toggle→渲染缩略图+词」。
