...
- 当前项目版本 `v0.15.0`：台词/旁白/心里话/角色名各新增「对齐」下拉框（左对齐/居中/首行缩进2字符），与字体、颜色并排放在对话主题的对应分区；修复切换阅读模式后立绘位置重置到初始的 bug（`rerenderActiveReader` 误用 `bridge.openMode` 覆盖当前模式、且读到错误模式桶的 spriteLayouts，现以活动阅读器 mode 为准并按该 mode 取桶）；修复切换 readerMode 后阅读器样式不立即生效（`readerMode` 分支补 `rerenderActiveReader`）；切换场景预设前弹窗确认，避免未保存进预设的改动被覆盖丢失；角色名↔对话框上边框间距收紧到 4px、角色名↔分割线 4px，撤掉上一版加在台词的负 margin。
