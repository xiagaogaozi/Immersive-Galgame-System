...
- 当前项目版本 `v0.17.0`：情绪词库与角色立绘 UI 合并——取消独立词库卡片，每个角色情绪槽折叠态显示「槽名+URL+操作」，展开后显示 72px 立绘缩略图（点击全屏放大预览）+该情绪组的词（词库全局共享）；槽名在词库无对应组时显「建为情绪组」按钮；新建情绪组/槽时组名自动作为该组第一个词；同名扫描：组名撞名仅阻止，词撞名弹窗询问「删重复词并移入」。
